...
import { fetchCryptoTicker, extractTickerData, ExtractedData, ApiResponse, localStorageLabel } from '../api/bitaps';
...
export const useCryptoTicker = (interval: number = 5000) => {
  ...
  useEffect(() => {
    ...
    chrome.storage.local.get(localStorageLabel, (storedData) => {
    ...
    const unsubscribe = $tickerData.watch((data: ApiResponse) => {
      ...
      chrome.storage.local.set({ [localStorageLabel]: data });
    });
